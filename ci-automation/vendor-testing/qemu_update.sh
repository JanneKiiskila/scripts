...
if [ -f tmp/flatcar_production_image_previous.bin ] ; then
    ...
    curl -fsSLO --retry-delay 1 --retry 60 --retry-connrefused --retry-max-time 60 --connect-timeout 20 "https://${CIA_CHANNEL}.release.flatcar.net/${CIA_ARCH}-usr/current/flatcar_production_image.bin.bz2"
